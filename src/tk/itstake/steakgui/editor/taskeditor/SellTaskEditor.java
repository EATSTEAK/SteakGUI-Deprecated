...
public class SellTaskEditor {
    public void show(Menu menu, Player player, int slot, int task) {
        ...
        setting.setItem(18, new ItemTaskItem(menu, player, task, 4, 1,  slot, SteakGUI.convertMessage("&c펄미션 지불"), Material.PAPER, new String[]{SteakGUI.convertMessage("&b아이템 판매시 펄미션을 지불합니다.")}));
        setting.setItem(19, new ItemTaskItem(menu, player, task, 5, 1,  slot, SteakGUI.convertMessage("&c아이템 지불"), Material.EMERALD, new String[]{SteakGUI.convertMessage("&b아이템 판매시 아이템을 지불합니다.")}));
        setting.setItem(20, new ItemTaskItem(menu, player, task, 6, 1,  slot, SteakGUI.convertMessage("&c돈 지불"), Material.IRON_INGOT, new String[]{SteakGUI.convertMessage("&b아이템 판매시 돈을 지불합니다.")}));
        if(((String)edittask.getData()[2]).equals("permission")) {
            setting.setItem(27, new ItemTaskItem(menu, player, task, 7, 1,  slot, SteakGUI.convertMessage("&b펄미션 입력"), Material.PAPER, new String[]{SteakGUI.convertMessage("&b아이템 판매시 소모할 펄미션을 입력합니다.")}));
        } else if(((String)edittask.getData()[2]).equals("money")) {
            setting.setItem(27, new ItemTaskItem(menu, player, task, 8, 1,  slot, SteakGUI.convertMessage("&b돈 액수 입력"), Material.IRON_INGOT, new String[]{SteakGUI.convertMessage("&b아이템 판매시 소모할 돈 액수를 입력합니다.")}));
        } else {
            setting.setItem(27, new ItemTaskItem(menu, player, task, 9, 1,  slot, SteakGUI.convertMessage("&b아이템 선택"), Material.EMERALD, new String[]{SteakGUI.convertMessage("&b아이템 판매시 소모할 아이템을 선택합니다.")}));
        }
        setting.setItem(36, new ItemTaskItem(menu, player, task, 10, 1,  slot, SteakGUI.convertMessage("&b판매 성공 메시지 입력"), Material.PAPER, new String[]{SteakGUI.convertMessage("&b판매에 성공하면 표시할 메시지를 입력합니다. 만약 '없음' 혹은 'none' 을 입력하면 메세지는 보이지 않습니다.")}));
        setting.setItem(37, new ItemTaskItem(menu, player, task, 11, 1,  slot, SteakGUI.convertMessage("&b돈 부족 메시지 입력"), Material.PAPER, new String[]{SteakGUI.convertMessage("&b판매에 실패하면 표시할 메시지를 입력합니다. 만약 '없음' 혹은 'none' 을 입력하면 메세지는 보이지 않습니다.")}));
    ...
    public void onChat(AsyncPlayerChatEvent e) {
        if(e.getPlayer().hasMetadata("permSell")) {
            ...
            new SellTaskEditor().show(menu, e.getPlayer(), (int) metadata[1], (int) metadata[2]);
            e.setCancelled(true);
    ...
    public void onItemClick(PlayerInteractEvent e) {
        if(e.getPlayer().hasMetadata("itemSell")) {
            if(e.getItem() != null) {
                ...
                e.setCancelled(true);
                new SellTaskEditor().show(menu, e.getPlayer(), (int) ((Object[]) e.getPlayer().getMetadata("itemSell").get(0).value())[1], (int) ((Object[]) e.getPlayer().getMetadata("itemSell").get(0).value())[2]);
            ...
            if(e.getItem() != null) {
                ...
                MenuFileHandler.saveMenu(menu);
                e.setCancelled(true);
                new SellTaskEditor().show(menu, e.getPlayer(), (int) ((Object[]) e.getPlayer().getMetadata("itemSellCost").get(0).value())[1], (int) ((Object[]) e.getPlayer().getMetadata("itemSellCost").get(0).value())[2]);
